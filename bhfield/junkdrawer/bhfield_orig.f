...
#ifdef USE_ARPREC
      integer MPDIGIT,MPDIGITG
      common /MPDIGIT/MPDIGIT
#endif
...
C
      I=0
      PRINT *, "you have inputed ", IARGC(), " values"
#ifdef USE_ARPREC

      
      IF((IARGC().NE.15).AND.(IARGC().NE.20)) THEN
       WRITE(*,*)
     1 'Usage: bhfield mpdigit wl[um] rad_core[um] rad_coat[um]',
...
C mpipl = Max prec level, digits; User sets in mp_mod.f
C     call mpgetpar('mpipl',MPIPL) -> no need
C     write(*,*) 'mpipl',mpipl

      IF(MPDIGITG.GT.MPIPL) THEN
       WRITE(*,*) 'mpdigit(',MPDIGITG,') > ',MPIPL
       STOP
      END IF
      call mpinit(MPDIGITG)
      call mpgetprec(MPDIGIT)
      IF(MPDIGIT.NE.MPDIGITG) THEN
       WRITE(*,*) 'Actual mpdigit:',MPDIGITG,'->',MPDIGIT
      END IF
...
C avoid radcor=0; bessel diverges
...
#ifdef USE_ARPREC
      WRITE(88,*) 'arprec version: mpdigit = ',MPDIGIT
#else
...
#ifdef USE_ARPREC
      IF(IARGC().EQ.15) THEN
       WRITE(51,*) 'bhfield ',MPDIGITG,WAVEL,RADCOR,RADCOT,
   ...
   11 FORMAT (/"COATED SPHERE SCATTERING: bhfield (version: ",A,
     1        1X,A,"; mpdigit = ",I4,")"/)
      WRITE(51,11) BHFIELD_VERSION,'arprec',MPDIGIT
#else
      IF(IARGC().EQ.14) THEN
...
#ifdef USE_ARPREC
 3324 FORMAT('mpdigit: used (',I4,') ',A,' required (',I4,
     1') -> ',A)
      IF(MPDIGIT.GE.MPDIGITR) THEN
       WRITE(88,3324) MPDIGIT,'>=',MPDIGITR,'seems OK'
      ELSE
       WRITE(88,3324) MPDIGIT,'<',MPDIGITR,'NG!!! Whoa!!!!'
       WRITE(*, 3324) MPDIGIT,'<',MPDIGITR,'NG!!! Whoa!!!!'
      END IF
#endif /* USE_ARPREC */
...
C
      ...
      integer MPDIGIT
      common /MPDIGIT/MPDIGIT
      type (mp_real) PSIBY,PSINY,ZERO,ONE,RN,X,Y
      type (mp_complex) X1,X2,Y2,REFREL,YC,RFREL1,RFREL2,CI
      type (mp_complex) CHIBX2,CHIBY2,CHINX2,CHINY2,DNX1,DNX2,DNY2
      type (mp_complex) XXIBY,XXINY
      type (mp_complex) PSINX1,PSINX2,PSINY2
      type (mp_complex) CHIPX2,CHIPY2,ANCAP,BNCAP,DNBAR,GNBAR
      type (mp_complex) CRACK,BRACK,AMESS1,AMESS2,AMESS3,AMESS4
      type (mp_complex) P,Q,R,S,T,U
      type (mp_complex) BJ(NMAX),BD(NMAX),BY(NMAX)
      type (mp_complex) DX1(NMAX),DX2(NMAX),DY2(NMAX)
      type (mp_complex) CHIX2(NMAX),CHIY2(NMAX)
      type (mp_complex) PSIX1(NMAX),PSIX2(NMAX),PSIY2(NMAX),XXIY(NMAX)
      type (mp_real) PSIY(NMAX),CHIY(NMAX)
      type (mp_complex) A(NMAX),B(NMAX),C(NMAX),D(NMAX)
      type (mp_complex) F(NMAX),G(NMAX),V(NMAX),W(NMAX)
#else
...
#ifdef USE_ARPREC
      type (mp_real) PSIPY
      type (mp_complex) PSIPX1,PSIPX2,PSIPY2,PSIBX1,PSIBX2,PSIBY2,XXIPY
#else
      REAL*8 PSIPY
...
#ifdef USE_ARPREC
       PSIY(N)=Y*mpreal(BJ(N))
       CHIY(N)=-(Y*mpreal(BY(N)))
#else
       PSIY(N)=Y*REAL(BJ(N))
       CHIY(N)=-(Y*REAL(BY(N)))
#endif
...
C Quick hack to check NaN
  ...
  822 FORMAT(A19,3('(',E12.5,',',E12.5,')'),' & ',
     1           3('(',E12.5,',',E12.5,')'))
#ifdef USE_ARPREC
      WRITE(51,821) 'PSIY,CHIY:',DBLE(PSIY(NP1)),DBLE(CHIY(NP1)),
     1                           DBLE(PSIY(NP2)),DBLE(CHIY(NP2))
      WRITE(51,822) 'BJ,BD,BY:',
     1              DBLE(mpreal(BJ(NP1))),DBLE(aimag(BJ(NP1))),
     2              DBLE(mpreal(BD(NP1))),DBLE(aimag(BD(NP1))),
     3              DBLE(mpreal(BY(NP1))),DBLE(aimag(BY(NP1))),
     4              DBLE(mpreal(BJ(NP2))),DBLE(aimag(BJ(NP2))),
     5              DBLE(mpreal(BD(NP2))),DBLE(aimag(BD(NP2))),
     6              DBLE(mpreal(BY(NP2))),DBLE(aimag(BY(NP2)))
      WRITE(51,822) 'PSIX1,PSIX2,PSIY2:',
     1              DBLE(mpreal(PSIX1(NP1))),DBLE(aimag(PSIX1(NP1))),
     2              DBLE(mpreal(PSIX2(NP1))),DBLE(aimag(PSIX2(NP1))),
     3              DBLE(mpreal(PSIY2(NP1))),DBLE(aimag(PSIY2(NP1))),
     4              DBLE(mpreal(PSIX1(NP2))),DBLE(aimag(PSIX1(NP2))),
     5              DBLE(mpreal(PSIX2(NP2))),DBLE(aimag(PSIX2(NP2))),
     6              DBLE(mpreal(PSIY2(NP2))),DBLE(aimag(PSIY2(NP2)))
      WRITE(51,822) 'CHIX2,CHIY2,XXIY:',
     1              DBLE(mpreal(CHIX2(NP1))),DBLE(aimag(CHIX2(NP1))),
     2              DBLE(mpreal(CHIY2(NP1))),DBLE(aimag(CHIY2(NP1))),
     3              DBLE(mpreal(XXIY(NP1))), DBLE(aimag(XXIY(NP1))),
     4              DBLE(mpreal(CHIX2(NP2))),DBLE(aimag(CHIX2(NP2))),
     5              DBLE(mpreal(CHIY2(NP2))),DBLE(aimag(CHIY2(NP2))),
     6              DBLE(mpreal(XXIY(NP2))), DBLE(aimag(XXIY(NP2)))
      WRITE(51,822) 'DX1,DX2,DY2:',
     1              DBLE(mpreal(DX1(NP1))),DBLE(aimag(DX1(NP1))),
     2              DBLE(mpreal(DX2(NP1))),DBLE(aimag(DX2(NP1))),
     3              DBLE(mpreal(DY2(NP1))),DBLE(aimag(DY2(NP1))),
     4              DBLE(mpreal(DX1(NP2))),DBLE(aimag(DX1(NP2))),
     5              DBLE(mpreal(DX2(NP2))),DBLE(aimag(DX2(NP2))),
     6              DBLE(mpreal(DY2(NP2))),DBLE(aimag(DY2(NP2)))
#else
...
C
      ...
      integer MPDIGIT
      common /MPDIGIT/MPDIGIT
      type (mp_real) ZERO,ONE,TWO,PI,RN
      type (mp_complex) CI,CREF1,WVK1,RHO1,BJN1,ENCAP,PSIP
      type (mp_complex) HFFACT
      type (mp_complex) BJ(NMAX)
      type (mp_complex) A(NMAX),B(NMAX),C(NMAX),D(NMAX)
      type (mp_complex) F(NMAX),G(NMAX),V(NMAX),W(NMAX)
      type (mp_complex) VM1O1N(3),VM1E1N(3),VN1O1N(3),VN1E1N(3)
      type (mp_complex) EF(3),HF(3)
#else
...
#ifdef USE_ARPREC_LEGENDRE
      type (mp_real) THETA,PHI
      type (mp_real) PIN(NMAX),TAU(NMAX)
#else
      REAL*8 THETA,PHI
...
C
      ...
      common /MPDIGIT/MPDIGIT
      type (mp_real) ZERO,ONE,TWO,PI,RN
      type (mp_complex) CI,CREF,WVK,RHO,ENCAP,XXIP,ZN
      type (mp_complex) HFFACT
      type (mp_complex) BJ(NMAX),BY(NMAX)
      type (mp_complex) A(NMAX),B(NMAX),C(NMAX),D(NMAX)
      type (mp_complex) F(NMAX),G(NMAX),V(NMAX),W(NMAX)
      type (mp_complex) VM3O1N(3),VM3E1N(3),VN3O1N(3),VN3E1N(3)
      type (mp_complex) EF(3),HF(3),EI(3),HI(3),ES(3),HS(3)
#else
      REAL*8 ZERO,ONE,TWO,PI,RN
      ...
      type (mp_real) THETA,PHI
      type (mp_real) PIN(NMAX),TAU(NMAX)
      type (mp_complex) HFFACTA,EIFAC,HIFAC
#else
      REAL*8 THETA,PHI
      REAL*8 PIN(NMAX),TAU(NMAX)
...
C
      ...
      common /MPDIGIT/MPDIGIT
      type (mp_real) ZERO,ONE,TWO,PI,RN
      type (mp_complex) CI,CREF2,WVK2,RHO2,BJN1,BYN1,ENCAP,PSIP,CHIP
      type (mp_complex) HFFACT
      type (mp_complex) BJ(NMAX),BY(NMAX)
      type (mp_complex) A(NMAX),B(NMAX),C(NMAX),D(NMAX)
      type (mp_complex) F(NMAX),G(NMAX),V(NMAX),W(NMAX)
      type (mp_complex) VM1O1N(3),VM1E1N(3),VN1O1N(3),VN1E1N(3)
      type (mp_complex) VM2O1N(3),VM2E1N(3),VN2O1N(3),VN2E1N(3)
      type (mp_complex) EF(3),HF(3)
#else
      REAL*8 ZERO,ONE,TWO,PI,RN
...
C       ************************************************
      ...
      type (mp_complex) X,CI,BJ(NMAX),BY(NMAX),BD(NMAX)
      type (mp_complex) XJ(0:LMAX),XJP(0:LMAX),
...
C       ************************************************
      ...
      integer MPDIGIT
      common /MPDIGIT/MPDIGIT
      type (mp_real) RN,ZERO,ONE,TWO,SMALL
      type (mp_complex) X,CNORM,TEMP,TEM2
      type (mp_complex) BJ(NMAX),BY(NMAX),BD(NMAX),T(3)
      type (mp_complex) AJ(NMAX),AY(NMAX),AD(NMAX),AP(NMAX)
#else
...
#ifdef USE_ARPREC
       WRITE(51,*) 'X = (',dble(mpreal(X)),',',dble(aimag(X)),')'
       DO 60 N=1,NORD+1
        WRITE(51,*) 'j_',N-1,'(X) = (',dble(mpreal(BJ(N))),',',
     1              dble(aimag(BJ(N))),')'
   60  CONTINUE
#else
       WRITE(51,*) 'X = ',X
...
#ifdef USE_ARPREC
      NST=NORD+1+AINT((101.0D0+mpreal(X))**0.5D0)
#else
...
#ifdef USE_ARPREC
     1 dble(mpreal(X)),dble(aimag(X)),N,dble(ABS(TEMP))
#else
     1 X,N,ABS(TEMP)
#endif
...
C limits
     ...
     1 'recurrence; expected -> y_n(0) = -INF'
       ...
       WRITE(51,*) 'X = (',dble(mpreal(X)),',',dble(aimag(X)),')'
       DO 20 N=1,NORD+1
        WRITE(51,*) 'y_',N-1,'(X) = (',dble(mpreal(BY(N))),',',
     1              dble(aimag(BY(N))),')'
   20  CONTINUE
#else
       WRITE(51,*) 'X = ',X
...
#ifdef USE_ARPREC
     1 dble(mpreal(X)),dble(aimag(X)),N,
     2 dble(mpreal(TEM2)),dble(aimag(TEM2)),dble(ABS(TEMP))
#else
     1 X,N,TEM2,ABS(TEMP)
#endif
...
#ifdef USE_ARPREC
     1 dble(mpreal(X)),dble(aimag(X)),N,
     2 dble(mpreal(AY(N))),dble(aimag(AY(N))),dble(ABS(TEMP))
#else
     1 X,N,AY(N),ABS(TEMP)
#endif
...
#ifdef USE_ARPREC
       WRITE(88,*) 'X = (',dble(mpreal(X)),',',dble(aimag(X)),')'
       DO 25 N=1,NORD+1
        WRITE(88,*) 'y_',N-1,'(X) = (',dble(mpreal(BY(N))),',',
     1              dble(aimag(BY(N))),')'
   25  CONTINUE
#else
       WRITE(88,*) 'X = ',X
...
C limits
     ...
     1 'recurrence; expected -> D_n(0) = INF'
       ...
       WRITE(51,*) 'X = (',dble(mpreal(X)),',',dble(aimag(X)),')'
       DO 220 N=1,NORD+1
        WRITE(51,*) 'D_',N-1,'(X) = (',dble(mpreal(BD(N))),',',
     1              dble(aimag(BD(N))),')'
  220  CONTINUE
#else
       WRITE(51,*) 'X = ',X
...
#ifdef USE_ARPREC
      NMX=AINT(MAX(mpreal(NORD),ABS(X)))+15
#else
...
C       ************************************************
      ...
      integer MPDIGIT
      common /MPDIGIT/MPDIGIT
      type (mp_real) TH,U,RN,ZERO,ONE,TWO
      type (mp_real) PIN(NMAX),TAU(NMAX)
#else
...
C ***
      ...
      integer MPDIGIT
      common /MPDIGIT/MPDIGIT
      type (mp_complex) X,CI,XI,W,PL,F,B,D,C,DEL,XJ0
      type (mp_complex) XJ(0:LMAX),XJP(0:LMAX),XH1(0:LMAX),
     1                  XH1P(0:LMAX)
      type (mp_real) ZERO,ONE,ACCUR,TM30,ABSC
#else
     ...
     1           XH1P(0:LMAX)
      ...
      ZERO='0.0D0'
      ONE='1.0D0'
      ACCUR=mpreal(10.0D0)**(-MPDIGIT)
      TM30=mpreal(10.0D0)**(-MPDIGIT)
#else
      ZERO=0.0D0
      ONE=1.0D0
      ACCUR=1.0D-12
...
#ifdef USE_ARPREC
       ABSC=ABS(mpreal(DEL-ONE))+ABS(AIMAG(DEL-ONE))
#else
